...
#For removing decoders and encoders, I think there are more but it's up to users, I hereby provide an example for my own opinion since we are using a better plane, can the GPU encoder can do the thing like us even with 10-bit HEVC? What I knew is, if for preview, this will work, but the final job requires CPU to do the final thing.
	config_options+=" --disable-decoder=*_cuvid --disable-decoder=*_qsv --disable-encoder=*_nvenc --disable-encoder=nvenc --disable-encoder=nvenc_* --disable-encoder=flv --disable-encoder=flashsv --disable-encoder=flashsv2"
