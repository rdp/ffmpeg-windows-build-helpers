...
build_librtmfp() {
  ...
  cd librtmfp_git
    if [[ $compiler_flavors != "native" ]]; then
      ...
      apply_patch file://$patch_dir/librtmfp_xp.diff -p1 # cross for windows needs it if on linux...
    else
...
build_ffmpeg() {
  ...
  output_dir+="_rtmfp"
  do_git_checkout https://github.com/thomasjammet/FFmpeg/ $output_dir $ffmpeg_git_checkout_version
