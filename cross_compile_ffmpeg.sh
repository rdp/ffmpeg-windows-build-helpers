...
check_missing_packages () {
  ...
  local check_packages=('curl' 'pkg-config' 'make' 'git' 'svn' 'gcc' 'autoconf' 'automake' 'yasm' 'cvs' 'flex' 'bison' 'makeinfo' 'g++' 'ed' 'hg' 'pax' 'unzip' 'patch' 'wget' 'xz' 'nasm' 'gperf' 'autogen')  
  ...
  if [[ -n "${missing_packages[@]}" ]]; then
    ...
    echo "for RHEL/CentOS: $ sudo yum install subversion texinfo mercurial cmake3 libtool autogen gperf nasm patch unzip pax ed gcc-c++ bison flex yasm automake autoconf gcc zlib-devel cvs -y"
