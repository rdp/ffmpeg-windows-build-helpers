...
# We will need this later if we don't want to just constantly be grepping the /etc/os-release file
	if [ -z "${VENDOR}" ] && grep -E '(centos|rhel)' /etc/os-release &> /dev/null;
	then
...
# case because the above code will assign "redhat" all the time.
	                            if [ -z "${VENDOR}" ] || [ "${VENDOR}" != "redhat" ] && [ "${VENDOR}" != "centos" ];
	then
...
do
	hash "$package" &> /dev/null || missing_packages=("$package" "${missing_packages[@]}")
		        ...
		        if [ "${VENDOR}" = "redhat" ] || [ "${VENDOR}" = "centos" ];
	then
	if [ -n "$(hash cmake 2>&1)" ] && [ -n "$(hash cmake3 2>&1)" ];
	then missing_packages=('cmake' "${missing_packages[@]}");
	fi
	fi
	...
	apt_pkgs='subversion ragel curl texinfo g++ ed libva-dev bison flex cvs yasm automake libtool autoconf gcc cmake git make pkg-config zlib1g-dev unzip pax nasm gperf autogen bzip2 autoconf-archive p7zip-full clang build-essential git-core libass-dev libfreetype6-dev libgnutls28-dev libmp3lame-dev libsdl2-dev libva-dev libvdpau-dev libvorbis-dev libxcb1-dev libxcb-shm0-dev libxcb-xfixes0-dev python-is-python3 meson autotools-dev gettext svn makeinfo patch wget xz realpath'
		...
		ubuntu_ver="$(lsb_release -rs)"
		           if at_least_required_version "18.04" "$ubuntu_ver";
			then
			apt_pkgs="$apt_pkgs python3-distutils" # guess it's no longer built-in, lensfun requires it...
			         fi
			         if at_least_required_version "20.04" "$ubuntu_ver";
				then
				apt_pkgs="$apt_pkgs python-is-python3" # needed
					...
					then
					echo NB if you use WSL Ubuntu 20.04 you need to do an extra step:
https://github.com/rdp/ffmpeg-windows-build-helpers/issues/452
							fi
							;;
							debian)
							echo "for debian:"
...
#
								...
								deb_ver="13"
									...
									apt_pkgs="$apt_pkgs python3-distutils" # guess it's no longer built-in, lensfun requires it...
									         ...
									         if at_least_required_version "11" "$deb_ver";
										then
										apt_pkgs="$apt_pkgs python-is-python3" # needed
										         fi
...
echo "for OS X (homebrew): brew install ragel wget cvs yasm autogen automake autoconf cmake libtool xz pkg-config nasm bzip2 autoconf-archive p7zip coreutils meson llvm" # if edit this edit docker/Dockerfile also :
											...
											export REQUIRED_CMAKE_VERSION="3.0.0"
											                              for cmake_binary in 'cmake' 'cmake3';
												do
...
# installed cmake at a higher version wouldn't be detected.
													if hash "${cmake_binary}"  &> /dev/null;
														then
														cmake_version="$( "$ {cmake_binary}" --version | sed -e "s#$ {cmake_binary}##g" | head -n 1 | tr -cd '[0-9.\n]' )"
														              if at_least_required_version "${REQUIRED_CMAKE_VERSION}" "${cmake_version}";
															then
...
# If cmake_command is set then either one of the cmake's is adequate.
																		if [[ $cmake_command != "cmake" ]];
																			then # don't echo if it's the normal default
																					...
																					export REQUIRED_YASM_VERSION="1.2.0" # export ???
																					                             local yasm_binary=yasm
																					                                     local yasm_version="$( "$ {yasm_binary}" --version |sed -e "s#$ {yasm_binary}##g" | head -n 1 | tr -dc '[0-9.\n]' )"
																					                                             if ! at_least_required_version "${REQUIRED_YASM_VERSION}" "${yasm_version}";
																						then
																						echo "your yasm version is too old $yasm_version wanted ${REQUIRED_YASM_VERSION}"
																						exit 1
																						fi
																						local meson_version=`meson --version`
																						                    if ! at_least_required_version "0.49.2" "${meson_version}";
																							then
...
# check WSL for kernel version look for version 4.19.128 current as of 11/01/2020
																							if uname -a | grep  -iq -- "-microsoft" ;
																								then
																								if cat /proc/sys/fs/binfmt_misc/WSLInterop | grep -q enabled ;
																									then
																										...
																										export MINIMUM_KERNEL_VERSION="4.19.128"
																										                              KERNVER=$(uname -a | awk -F'[ ]' '{ print $3 }' | awk -F- '{ print $1 }')
																										                                      ...
																										                                      function version { # for version comparison @ stackoverflow.com/a/37939589
																											                                      ...
																											                                      if [ $(version $KERNVER) -lt $(version $MINIMUM_KERNEL_VERSION) ];
																												then
...
##################### Welcome ######################
	...
	mkdir -p "$cur_dir"
	cd "$cur_dir"
	if [[ $disable_nonfree = "y" ]];
	then
	non_free="n"
	         else if  [[ $disable_nonfree = "n" ]];
	then
	non_free="y"
	         else
...
# --disable-shared allows c++ to be distributed at all...which seemed necessary for some random dependency which happens to use/require c++...
	    local zeranoe_script_name=mingw-w64-build-r22.local
	                              local zeranoe_script_options="--gcc-ver=10.2.0 --mingw-w64-ver=9.0.0 --default-configure --cpu-count=$gcc_cpu_count --disable-shared --clean-build --verbose --allow-overwrite --threads=winpthreads" # allow-overwrite to avoid some crufty prompts if I do rebuilds [or maybe should just nuke everything...]
			                                      if [[ ($compiler_flavors == "win32" || $compiler_flavors == "multi") && ! -f ../$win32_gcc ]];
	then
	echo "Building win32 cross compiler..."
	download_gcc_build_script $zeranoe_script_name
	if [[ `uname` =~ "5.1" ]];
	then # Avoid using secure API functions for compatibility with msvcrt.dll on Windows XP.
	...
	if [[ ! -f  ../cross_compilers/mingw-w64-i686/i686-w64-mingw32/lib/libmingwex.a ]];
	then
	echo "failure building mingwex? 32 bit"
	exit 1
	fi
	fi
	if [[ ($compiler_flavors == "win64" || $compiler_flavors == "multi") && ! -f ../$win64_gcc ]];
	then
	...
	if [[ ! -f  ../cross_compilers/mingw-w64-x86_64/x86_64-w64-mingw32/lib/libmingwex.a ]];
	then
...
retry_git_or_die() {
# originally from https://stackoverflow.com/a/76012343/32453
	local RETRIES_NO=50
	                 local RETRY_DELAY=3
	                                   local repo_url=$1
	                                           local to_dir=$2

	                                                   for i in $(seq 1 $RETRIES_NO);
	do
...
# Check for either "origin/main" or "origin/master".
	if [ $(git show-ref | grep -e origin\/main$ -c) = 1 ];
	then
	desired_branch="origin/main"
	               elif [ $(git show-ref | grep -e origin\/master$ -c) = 1 ];
	then
...
# vmaf on 16.04 needed that weird reset --hard? huh?
		               if git show-ref --verify --quiet "refs/remotes/origin/$desired_branch";
then # $desired_branch is actually a branch, not a tag or commit
...
git_hard_reset() {
	...
	git clean -fx # throw away local changes; 'already_*'
	and bak-files for instance.
}

get_small_touchfile_name() {
# have to call with assignment like a=$(get_small...)
	local beginning="$1"
	                local extra_stuff="$2"
...
echo "configuring $english_name ($PWD) as $ PKG_CONFIG_PATH=$PKG_CONFIG_PATH PATH=$mingw_bin_path:\$PATH $configure_name $configure_options" # say it now in case bootstrap fails etc.
	...
	if [[ ! -f $configure_name && -f bootstrap.sh ]];
then # fftw wants to only run this if no configure :
	|
	./bootstrap.sh
	fi
	if [[ ! -f $configure_name ]];
...
do_make_install() {
	local extra_make_install_options="$1"
	                                 local override_make_install_options="$2" # startingly, some need/use something different than just 'make install'
	                                         if [[ -z $override_make_install_options ]];
	then
...
# some sources don't allow it, weird XXX combine with the above :)
	source_dir="$1"
	           extra_args="$2"
...
generic_configure() {
	build_triple="${build_triple:-$(gcc -dumpmachine)}"
	             local extra_configure_options="$1"
	                     if [[ -n $build_triple ]];
	then extra_configure_options+=" --build=$build_triple";
	fi
...
gen_ld_script() {
	lib=$mingw_w64_x86_64_prefix/lib/$1
	    lib_s="$2"
	          if [[ ! -f $mingw_w64_x86_64_prefix/lib/lib$lib_s.a ]];
	then
...
apply_patch file://$patch_dir/bzip2-1.0.8_brokenstuff.diff
	if [[ ! -f ./libbz2.a ]] || [[ -f $mingw_w64_x86_64_prefix/lib/libbz2.a && ! $(/usr/bin/env md5sum ./libbz2.a) = $(/usr/bin/env md5sum $mingw_w64_x86_64_prefix/lib/libbz2.a) ]];
	then # Not built or different build installed
...
apply_patch file://$patch_dir/SDL2-2.0.12_lib-only.diff
	...
	export CFLAGS="$CFLAGS -DDECLSPEC="  # avoid SDL trac tickets 939 and 282 [broken shared builds]
	              if [[ $compiler_flavors == "native" ]];
	then
	unset PKG_CONFIG_LIBDIR # Allow locally installed things for native builds;
libpulse-dev is an important one otherwise no audio for most Linux
	fi
	...
	export PKG_CONFIG_LIBDIR=
	    ...
	    if [[ ! -f $mingw_bin_path/$host_target-sdl2-config ]];
	then
...
do_git_checkout https://github.com/GPUOpen-LibrariesAndSDKs/AMF.git amf_headers_git
	...
	cd amf_headers_git
	if [ ! -f "already_installed" ];
	then
#rm -rf "./Thirdparty" # ?? plus too chatty...
	if [ ! -d "$mingw_w64_x86_64_prefix/include/AMF" ];
	then
...
build_intel_qsv_mfx() {
# disableable via command line switch...
do_git_checkout https://github.com/lu-zero/mfx_dispatch.git mfx_dispatch_git 2cd279f # lu-zero?? oh well seems somewhat supported...
	...
	if [[ $compiler_flavors == "native" && $OSTYPE != darwin* ]];
	then
	                                ...
	                                export CXXFLAGS="$CFLAGS" # Not certain this is needed, but it doesn't hurt
	                                        export LDFLAGS="-L${mingw_w64_x86_64_prefix}/lib" # For some reason the frexp configure checks fail without this as math.h isn't found when cross-compiling; no negative impact for native builds
	                                                local meson_options="--prefix=${mingw_w64_x86_64_prefix} --libdir=${mingw_w64_x86_64_prefix}/lib --buildtype=release --default-library=static -Dinternal_pcre=true -Dforce_posix_threads=true . build"
	                                                        if [[ $compiler_flavors != "native" ]];
	then
	get_local_meson_cross_with_propeties # Need to add flags to meson properties;
	otherwise ran into some issues
...
build_harfbuzz() {
	local new_build=false
                do_git_checkout https://github.com/harfbuzz/harfbuzz.git harfbuzz_git "93930fb1c49b85" # keep old autogen build :)
	                if [ ! -f harfbuzz_git/already_done_harf ];
	then # Not done or new master, so build
	new_build=true
	          fi
...
build_freetype() {
	local force_build=$2
	                  local new_build=1
	                                  if [[ ! -f freetype-2.10.4/already_done_freetype || $force_build = true ]];
	then
...
do_git_checkout https://github.com/Netflix/vmaf.git vmaf_git v2.3.0
	...
	meson_options+=" --cross-file=meson-cross.mingw.txt"
	               ...
	               unset LDFLAGS
	               if [[ $compiler_flavors == "native" ]];
	then # Can't find a way to not build these; meson is already set to --default-library=static but it still builds both
...
do_git_checkout https://github.com/libsndfile/libsndfile.git
	...
	if [ "$1" = "install-libgsm" ];
	then
	if [[ ! -f $mingw_w64_x86_64_prefix/lib/libgsm.a ]];
	then
...
do_git_checkout https://github.com/njh/twolame.git twolame_git "origin/main"
	cd twolame_git
	if [[ ! -f Makefile.am.bak ]];
	then # Library only, front end refuses to build for some reason with git master
...
build_fdk-aac() {
	local checkout_dir=fdk-aac_git
	                   if [[ ! -z $fdk_aac_git_checkout_version ]];
	then
	...
	git submodule update --init --remote # For UDF support [default=enabled], which strangely enough is in another repository.
		...
		local local_git_version=`git --git-dir=.git/modules/contrib/libudfread rev-parse HEAD`
                                       local remote_git_version=`git ls-remote -h https://code.videolan.org/videolan/libudfread.git | sed "s/[[:space:]].*//"`
		                                               if [[ "$local_git_version" != "$remote_git_version" ]];
	then
	echo "detected upstream udfread changed, attempted to update submodules" # XXX use do_git_checkout here instead somehow?
	git submodule foreach -q 'git clean -fx' # Throw away local changes; 'already_configured_*'
	and 'udfread.c.bak' in this case.
...
do_git_checkout https://github.com/pkuvcl/davs2.git
	...
	if [[ $host_target == 'i686-w64-mingw32' ]];
	then
...
# VP8 encoder *requires* sse3 support
                                     do_configure "$config_options --prefix=$mingw_w64_x86_64_prefix --enable-ssse3 --enable-static --disable-shared --disable-examples --disable-tools --disable-docs --disable-unit-tests --enable-vp9-highbitdepth --extra-cflags=-fno-asynchronous-unwind-tables --extra-cflags=-mstackrealign" # fno for Error:
...
do_git_checkout https://code.videolan.org/videolan/dav1d.git libdav1d
	cd libdav1d
	if [[ $bits_target == 32 || $bits_target == 64 ]];
	then # XXX why 64???
...
do_cmake_from_build_dir .. -DHEADERS_ONLY:
	bool=on
...
# Apply x86 noasm detection fix on newer versions
	                   if [[ $x265_git_checkout_version != *"3.5"* ]] && [[ $x265_git_checkout_version != *"3.4"* ]] && [[ $x265_git_checkout_version != *"3.3"* ]] && [[ $x265_git_checkout_version != *"3.2"* ]] && [[ $x265_git_checkout_version != *"3.1"* ]];
	then
...
# Build 8 bit (main) with linked 10 and 12 bit then install
	                   ...
	                   cmake_params="$cmake_params -DENABLE_CLI=1 -DEXTRA_LINK_FLAGS=-L. -DLINKED_10BIT=1 -DLINKED_12BIT=1"
	                                if [[ $compiler_flavors == "native" && $OSTYPE != darwin* ]];
	then
	cmake_params+=" -DENABLE_SHARED=0 -DEXTRA_LIB='$(pwd)/libx265_main10.a;$(pwd)/libx265_main12.a;-ldl'" # Native multi-lib CLI builds are slightly broken right now;
	other option is to -DENABLE_CLI=0, but this seems to work (https://bitbucket.org/multicoreware/x265/issues/520)
	                                else
...
generic_configure "CFLAGS=-DHAVE_DVDCSS_DVDCSS_H LDFLAGS=-ldvdcss --enable-dlfcn" # vlc patch: "--enable-libdvdcss"
# XXX ask how I'm *supposed* to do this to the dvdread peeps [svn?]
	do_make_and_make_install
...
build_ffmpeg() {
	...
	output_dir+="_lgpl"
	            ...
	            if [[ ! -z $ffmpeg_git_checkout_version ]];
	then
...
# allow using local source directory version of ffmpeg
	...
	postpend_configure_opts="--enable-shared --disable-static --prefix=${install_prefix}" # I guess this doesn't have to be at the end...
					                        ...
					                        postpend_configure_opts="--enable-static --disable-shared --prefix=${install_prefix}"
					                                ...
					                                if [[ $ffmpeg_git_checkout_version == *"n4.4"* ]] || [[ $ffmpeg_git_checkout_version == *"n4.3"* ]] || [[ $ffmpeg_git_checkout_version == *"n4.2"* ]];
	then
	                                                                                                                                                                                                     ...
	                                                                                                                                                                                                     config_options+=" --enable-opengl"
	                                                                                                                                                                                                             config_options+=" --enable-libdav1d"
	                                                                                                                                                                                                                     config_options+=" --enable-gnutls"
	                                                                                                                                                                                                                             config_options+=" --enable-libxevd"
	                                                                                                                                                                                                                                     config_options+=" --enable-libxeve"
	                                                                                                                                                                                                                                             config_options+=" --enable-gdigrab"
	                                                                                                                                                                                                                                                     config_options+=" --enable-d3d12va"
	                                                                                                                                                                                                                                                             config_options+=" --enable-vvc"
	                                                                                                                                                                                                                                                                     config_options+=" --enable-quirc"
	                                                                                                                                                                                                                                                                                             ...
	                                                                                                                                                                                                                                                                                             config_options+=" --enable-filter=tiltshift"
	                                                                                                                                                                                                                                                                                                     config_options+=" --enable-filter=qrencode"
	                                                                                                                                                                                                                                                                                                             config_options+=" --enable-filter=qrencodesrc"
	                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-filter=fsync"
	                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-filter=aap"
	                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-muxer=iamf"
	                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-demuxer=iamf"
	                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-muxer=rawvideo"
	                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-demuxer=rawvideo"
	                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-muxer=rawvideo"
	                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-demuxer=rawvideo"
	                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-muxer=rcwt"
	                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-demuxer=rcwt"
	                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-muxer=mp4"
	                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-demuxer=mp4"
	                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-muxer=heif"
	                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-demuxer=heif"
	                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-muxer=avif"
	                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-demuxer=avif"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-muxer=dvd"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-demuxer=dvd"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-muxer=flv"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-demuxer=flv"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-muxer=dnn"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-demuxer=dnn"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-muxer=loopback"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-demuxer=loopback"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-muxer=qoa"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-demuxer=qoa"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-filter=dnn"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-filter=showinfo"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-filter=randomi"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-filter=bsf"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-filter=export_side_data"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-filter=film_grain"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-filter=loopback"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-hwaccel=vulkan"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-protocol=android_content_uris"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-protocol=packettypemetadata"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=dxv"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=dxt1"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=mcmp"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=evc"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=qoa"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=hevc"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=vp9"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=av1"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=mpeg2"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=vc1"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=tiltshift"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=quirc"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=fsync"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=rawvideo"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=rcwt"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=bsf"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=showinfo"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=film_grain"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=loopback"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=dnn"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=aap"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=iamf"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=randomi"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=qrencode"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=qrencodesrc"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=tiltshift"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=quirc"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=dxv"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=dxt1"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     ...
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=hevc"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=vp9"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=av1"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-codec=mpeg2"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-codec=vc1"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=dxv"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-decoder=dxt1"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=mcmp"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-decoder=evc"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=qoa"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-decoder=hevc"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=vp9"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-decoder=av1"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=mpeg2"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-decoder=vc1"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=tiltshift"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-decoder=quirc"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=fsync"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-decoder=rawvideo"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=rcwt"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-decoder=bsf"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=showinfo"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-decoder=film_grain"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=loopback"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-decoder=dnn"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=aap"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-decoder=iamf"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=randomi"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-decoder=qrencode"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=qrencodesrc"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     config_options+=" --enable-decoder=tiltshift"
	                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             config_options+=" --enable-decoder=quirc"
...
# https://github.com/OpenVisualCloud/SVT-HEVC/commit/b5587b09f44bcae70676f14d3bc482e27f07b773#diff-2b35e92117ba43f8397c2036658784ba2059df128c9b8a2625d42bc527dffea1
	...
	elif [[ $ffmpeg_git_checkout_version == *"n4.1"* ]] || [[ $ffmpeg_git_checkout_version == *"n3.4"* ]] || [[ $ffmpeg_git_checkout_version == *"n3.2"* ]] || [[ $ffmpeg_git_checkout_version == *"n2.8"* ]];
	then
:
# too old...
	else
...
# the order of extra-libs switches is important (appended in reverse)
	...
	config_options+=" --disable-amf" # Since its autodetected we have to disable it if we do not want it. #unless we define no autodetection but.. we don't.
							                ...
							                config_options+=" --enable-amf" # This is actually autodetected but for consistency.. we might as well set it.
								                                ...
								                                if [[ $build_intel_qsv = y && $compiler_flavors != "native" ]];
then # Broken for native builds right now:
https://github.com/lu-zero/mfx_dispatch/issues/71
...
# other possible options: --enable-openssl [unneeded since we already use gnutls]
                ...
                do_debug_build=n # if you need one for backtraces/examining segfaults using gdb.exe ... change this to y :
                                               ) XXXX make it affect x264 too...and make it real param :
										                                                                                                                                    )
										if [[ "$do_debug_build" = "y" ]];
											then
...
# bash recursive glob fails here again?
	      for file in `find . -name vlc.exe | grep -- -`;
do
	found="$found $(readlink -f $file)"
	      done
...
build_ffmpeg_dependencies() {
	...
	return
	    ...
	    if [[ $compiler_flavors != "native" ]];
	then # build some stuff that don't build native...
	build_dlfcn
	build_libxavs
	fi
...
# now the things that use the dependencies...
	...
	if [[ $build_ffmpeg_static = "y" ]];
	then
	build_ffmpeg static
	fi
	if [[ $build_ffmpeg_shared = "y" ]];
	then
	build_ffmpeg shared
	fi
	if [[ $build_vlc = "y" ]];
...
cpu_count=1 # else default to just 1, instead of blank, which means infinite
	          ...
	          set_box_memory_size_bytes
	          if [[ $box_memory_size_bytes -lt 600000000 ]];
then
...
if [[ $box_memory_size_bytes -gt 2000000000 ]];
then
	                                                                                            ...
	                                                                                            build_dependencies=y
	                                                                                                    git_get_latest=y
	                                                                                                            prefer_stable=y # Only for x264 and x265.
                                                                                                                    build_intel_qsv=y # note:
	                                                                                                                            not windows xp friendly!
	                                                                                                                            build_amd_amf=y
...
shift ;;
--ffmpeg-git-checkout=* ) ffmpeg_git_checkout="${1#*=}";
shift ;;
--ffmpeg-source-dir=* ) ffmpeg_source_dir="${1#*=}";
shift ;;
--x265-git-checkout-version=* ) x265_git_checkout_version="${1#*=}";
shift ;;
--fdk-aac-git-checkout-version=* ) fdk_aac_git_checkout_version="${1#*=}";
shift ;;
--build-libmxf=* ) build_libmxf="${1#*=}";
shift ;;
--build-mp4box=* ) build_mp4box="${1#*=}";
shift ;;
--build-ismindex=* ) build_ismindex="${1#*=}";
shift ;;
--git-get-latest=* ) git_get_latest="${1#*=}";
shift ;;
--build-amd-amf=* ) build_amd_amf="${1#*=}";
shift ;;
--build-intel-qsv=* ) build_intel_qsv="${1#*=}";
shift ;;
--build-x264-with-libav=* ) build_x264_with_libav="${1#*=}";
shift ;;
--build-mplayer=* ) build_mplayer="${1#*=}";
shift ;;
--cflags=* )
original_cflags="${1#*=}";
echo "setting cflags as $original_cflags";
shift ;;
--build-vlc=* ) build_vlc="${1#*=}";
shift ;;
--build-lsw=* ) build_lsw="${1#*=}";
shift ;;
--build-dvbtee=* ) build_dvbtee="${1#*=}";
shift ;;
--disable-nonfree=* ) disable_nonfree="${1#*=}";
shift ;;
...
-a         ) compiler_flavors="multi";
    ...
    build_intel_qsv=y;
    build_dvbtee=y;
    build_x264_with_libav=y;
    shift ;;
--build-svt-hevc=* ) build_svt_hevc="${1#*=}";
    shift ;;
--build-svt-vp9=* ) build_svt_vp9="${1#*=}";
    shift ;;
-d         ) echo "defaults: doing 64 bit only, fast";
...
build_intel_qsv=n
                ...
                shift ;;
--build-ffmpeg-static=* ) build_ffmpeg_static="${1#*=}";
    shift ;;
--build-ffmpeg-shared=* ) build_ffmpeg_shared="${1#*=}";
    shift ;;
--prefer-stable=* ) prefer_stable="${1#*=}";
    shift ;;
...
#  bs2b doesn't use pkg-config, sndfile needed Carbon :|
								                        export CPATH=$cur_dir/cross_compilers/native/include:
								                                     /Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/System/Library/Frameworks/Carbon.framework/Versions/A/Headers # C_INCLUDE_PATH
								                                     export LIBRARY_PATH=$cur_dir/cross_compilers/native/lib
								                                             work_dir="$(realpath $cur_dir/native)"
								                                                     ...
								                                                     if [[ $compiler_flavors == "multi" || $compiler_flavors == "win32" ]];
									                                                     then
									                                                     echo
									                                                                    ...
									                                                                    cross_prefix="$mingw_bin_path/i686-w64-mingw32-"
									                                                                            make_prefix_options="CC=${cross_prefix}gcc AR=${cross_prefix}ar PREFIX=$mingw_w64_x86_64_prefix RANLIB=${cross_prefix}ranlib LD=${cross_prefix}ld STRIP=${cross_prefix}strip CXX=${cross_prefix}g++"
									                                                                                    work_dir="$(realpath $cur_dir/win32)"
									                                                                                            ...
									                                                                                            if [[ $compiler_flavors == "multi" || $compiler_flavors == "win64" ]];
										                                                                                            then
										                                                                                            echo
										                                                                    ...
										                                                                    cross_prefix="$mingw_bin_path/x86_64-w64-mingw32-"
										                                                                            make_prefix_options="CC=${cross_prefix}gcc AR=${cross_prefix}ar PREFIX=$mingw_w64_x86_64_prefix RANLIB=${cross_prefix}ranlib LD=${cross_prefix}ld STRIP=${cross_prefix}strip CXX=${cross_prefix}g++"
										                                                                                    work_dir="$(realpath $cur_dir/win64)"
										                                                                                            ...
										                                                                                            for file in $(find_all_build_exes);
											                                                                                            do
												                                                                                            echo "built $file"
												                                                                                            done
