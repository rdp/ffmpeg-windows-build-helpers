...
build_ffmpeg() {
  ...
  cd $output_dir
    ...
    if [[ $enable_gpl == 'y' ]]; then
      config_options+=" --enable-gpl --enable-avisynth --enable-frei0r --enable-filter=frei0r --enable-librubberband --enable-libvidstab --enable-libx264 --enable-libx265 --enable-libxavs --enable-libxvid --disable-hwaccels --enable-hwaccel=dxva2 --enable-hwaccel=d3d11va --disable-encoder=libmp3lame --disable-encoder=libxpeedx --disable-encoder=libtheora --disable-encoder=libtwolame --disable-encoder=libxvid --disable-encoder=libvpx_vp8"
    fi
