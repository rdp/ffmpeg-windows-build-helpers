...
# This is for purely progressive for the finest codecs, I still am tuning, hoping the complication is getting better. It seems there may be few people have added something in libavcodec...

#The script below will be only generate progressive only content except mpeg-2, also the default configuration below are for 64-bit OS only, if you have interlaced content, then please use yadif=1:-1:1 to get correctly first. I personally, trying to find if there are true pure progressive function, 23.976, 24, 25, 50, 60 are my usually number.
...
do_cmake() {
  ...
  if [ ! -f $touch_name ]; then
    ...
    echo cmake GUnix Makefiles . -DENABLE_STATIC_RUNTIME=1 -DCMAKE_SYSTEM_NAME=Windows -DCMAKE_RANLIB=${cross_prefix}ranlib -DCMAKE_C_COMPILER=${cross_prefix}gcc -DCMAKE_CXX_COMPILER=${cross_prefix}g++ -DCMAKE_RC_COMPILER=${cross_prefix}windres -DCMAKE_INSTALL_PREFIX=$mingw_w64_x86_64_prefix $extra_args
    cmake GUnix Makefiles . -DENABLE_STATIC_RUNTIME=1 -DCMAKE_SYSTEM_NAME=Windows -DCMAKE_RANLIB=${cross_prefix}ranlib -DCMAKE_C_COMPILER=${cross_prefix}gcc -DCMAKE_CXX_COMPILER=${cross_prefix}g++ -DCMAKE_RC_COMPILER=${cross_prefix}windres -DCMAKE_INSTALL_PREFIX=$mingw_w64_x86_64_prefix $extra_args || exit 1
    touch $touch_name || exit 1
  fi
}
...
#For removing encoders and other options to suit needs
	config_options+=" --disable-encoder=*nvenc* --disable-encoder=libopenh264 --disable-encoder=*mp3* --disable-encoder=*amr* --disable-encoder=*adpcm* "
